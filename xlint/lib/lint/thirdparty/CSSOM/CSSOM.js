...
CSSOM.parse = function parse(token) {
	...
	*/
	...
	// @type CSSStyleSheet|CSSMediaRule|CSSKeyframesRule|CSSDocumentRule
	...
	for (var character; character = token.charAt(i); i++) {
		...
		case "/":
			if (token.charAt(i + 1) === "*") {
				...
				if (index === -1) {
					...
					comment = token.substring(i, index).trim().toLowerCase();

					// start handle xlint inline config
					if (comment.indexOf("xlint") !== -1 && currentScope) {
						if (comment === "xlint.ignorerule") {
							if (state === "before-name") {
								styleRule.ignoreXLint = true;
							} else if (state === "keyframeRule-begin") {
								currentScope.ignoreXLint = true;
							} else if (state === "before-selector") {
								if (currentScope !== styleSheet) {
									currentScope.ignoreXLint = true;
								}
							}
						} else if (comment.indexOf("xlint.ignoreproperty") !== -1) {
							commentArgs = comment.substring("xlint.ignoreproperty".length, comment.length).trim();
							if (state === "before-selector" ||
								state === "keyframeRule-begin") {
								if (commentArgs) {
									commentArgs.split(',').forEach(function(prop) {
										prop = prop.trim();
										if (prop) {
											if (!currentScope.ignoredPropsByXLint) {
												currentScope.ignoredPropsByXLint = [prop];
											} else {
												currentScope.ignoredPropsByXLint.push(prop);
											}
										}
									});
								}
							}
							else if (state === "before-name") {
								if (!commentArgs) {
									if (!styleRule.ignoredRangesByXLint) {
										styleRule.ignoredRangesByXLint = [{
											from: styleRule.style.length,
											to:   styleRule.style.length
										}];
									} else {
										styleRule.ignoredRangesByXLint.push({
											from: styleRule.style.length,
											to:   styleRule.style.length
										})
									}
								} else if (/^\d+$/.test(commentArgs)) {
									if (!styleRule.ignoredRangesByXLint) {
										styleRule.ignoredRangesByXLint = [{
											from: styleRule.style.length,
											to:   styleRule.style.length + parseInt(commentArgs) - 1
										}];
									} else {
										styleRule.ignoredRangesByXLint.push({
											from: styleRule.style.length,
									...
									commentArgs.split(',').forEach(function(prop) {
										prop = prop.trim();
										if (prop) {
											if (!styleRule.ignoredPropsByXLint) {
												styleRule.ignoredPropsByXLint = [prop];
											} else {
												styleRule.ignoredPropsByXLint.push(prop);
											}
										}
									});
								}
					...
					// end handle xlint inline config
					i = index + 1;
				}
			} else {
				buffer += character;
		...
		case ";":
			switch (state) {
				case "value":
					styleRule.style.setProperty(name, buffer.trim(), priority, propertyStarts, i);
					priority = "";
					buffer = "";
					state = "before-name";
					break;
